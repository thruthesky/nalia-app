...
class UserCardController extends GetxController {
  ...
  void onInit() {
    ...
    withcenterApi.locationChanges.listen((re) async {
      if (re == null) return;
  ...
  fetchUsers() async {
    print("-----> location ready: ${withcenterApi.locationReady}");
  ...
  Future<List<ApiBio>> search() async {
    ...
    if (withcenterApi.locationReady) {
      latitude = withcenterApi.myLocation.latitude;
      longitude = withcenterApi.myLocation.longitude;
    ...
    // print('req: $req');
    final re = await withcenterApi.request(req);
